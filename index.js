...
(async function () {
    ...
    function wrapConsoleForDebug() {
        if (window.__TA_DEBUG_CONSOLE_WRAPPED__) return;
        window.__TA_DEBUG_CONSOLE_WRAPPED__ = true;
        ...
        console.log = (...args) => {
            try {
                if (args && args.length > 0 && String(args[0]).includes(LOG)) {
                    pushDebugLog('log', args.map(a => {
    ...
    function maskText(value, keepStart = 2, keepEnd = 2) {
        ...
        if (s.length <= keepStart + keepEnd) return '*'.repeat(Math.max(1, s.length));
        return `${s.slice(0, keepStart)}***${s.slice(-keepEnd)}`;
    }

    function maskCoord(value) {
    ...
    function sanitizeSettingsForDiag(settings) {
        const s = structuredClone(settings);

        if (s.weatherLat !== undefined && s.weatherLat !== '') s.weatherLat = maskCoord(s.weatherLat);
        if (s.weatherLon !== undefined && s.weatherLon !== '') s.weatherLon = maskCoord(s.weatherLon);
        if (typeof s.weatherLocationText === 'string' && s.weatherLocationText) s.weatherLocationText = maskText(s.weatherLocationText, 2, 0);

        if (Array.isArray(s.autoMessageCharacters)) {
            s.autoMessageCharacters = s.autoMessageCharacters.map((x, i) => `char_${i + 1}`);
        }
        if (Array.isArray(s.injectCharacters)) {
            s.injectCharacters = s.injectCharacters.map((x, i) => `char_${i + 1}`);
        }

        return s;
    }

    function sanitizeCacheForDiag() {
        return {
            weatherCache: {
                ...weatherCache,
            ...
            charWeatherCache: {
                ...charWeatherCache,
                locationName: charWeatherCache.locationName ? maskText(charWeatherCache.locationName, 2, 0) : '',
                lat: charWeatherCache.lat !== null ? maskCoord(charWeatherCache.lat) : null,
                lon: charWeatherCache.lon !== null ? maskCoord(charWeatherCache.lon) : null,
                markerRaw: charWeatherCache.markerRaw ? maskText(charWeatherCache.markerRaw, 8, 2) : '',
            },
            nagerCache,
            nagerCountriesCache: {
                ...nagerCountriesCache,
                list: Array.isArray(nagerCountriesCache.list) ? nagerCountriesCache.list.length : 0,
            },
        };
    }
    ...
    function collectDiagnostics() {
        ...
        const currentChar = (ctx.characterId !== undefined && ctx.characterId !== null && ctx.characters)
            ? ctx.characters[ctx.characterId]
            : null;

        const macroName = sanitizeMacroName(settings.macroName || 'time_awareness');
        const macroToken = getMacroToken(macroName);
        ...
        const diag = {
            ...
            plugin: {
                ...
                nagerDetecting,
                isAutoGenerating,
                mainTimerRunning: !!mainTimerHandle,
                langWatcherRunning: !!langWatcher,
            },
            environment: {
            ...
            chat: {
                chatId: ctx.getCurrentChatId ? ctx.getCurrentChatId() : null,
                characterId: ctx.characterId,
                characterNameMasked: currentChar ? maskText(currentChar.name || 'unknown', 1, 0) : '',
                messageCount: Array.isArray(ctx.chat) ? ctx.chat.length : 0,
                lastUserMessageTime,
                idleTriggeredThisPeriod,
                lastIdleRollTime,
            },
            macro_diagnosis: {
                ...
                macroToken,
                registeredMacroName,
                isMacroNameMatched: registeredMacroName === macroName,
                hint: settings.injectionMode === 'macro'
                    ? '宏模式下需要确认预设/世界书/系统模板中真实存在该占位符'
                    : '当前不是宏模式，宏占位符不会被替换',
            },
            settings_snapshot: sanitizeSettingsForDiag(settings),
            cache_snapshot: sanitizeCacheForDiag(),
            last_errors: {
                lastAutoSaveFailed,
                lastAutoSaveError,
                lastAutoGenError,
            },
            recent_logs: debugLogs.slice(-200),
            preview_prompt: buildTimePromptText(),
        };

        return diag;
    }

    function diagnosticsToText(diagObj) {
        return [
            '===== Time Awareness Diagnostics =====',
            safeStringify(diagObj),
            '===== End Diagnostics ====='
        ].join('\n');
    }
    ...
    function downloadDiagnostics(text) {
        ...
        const ts = new Date();
        const filename = `time-awareness-diagnostics-${ts.getFullYear()}${pad2(ts.getMonth() + 1)}${pad2(ts.getDate())}-${pad2(ts.getHours())}${pad2(ts.getMinutes())}${pad2(ts.getSeconds())}.txt`;
...
<div id="ta_settings" class="inline-drawer">
    ...
    <div class="inline-drawer-content">
        ...
        <div class="settings_section">
            ...
            <select id="ta_inject_character_select" class="text_pole" multiple></select>
        </div>
        ...
        <div class="settings_section">
            ...
            <div class="ta_test_row">
                <div id="ta_btn_test_special" class="menu_button ta-inline-btn" style="font-size:0.85em;">${t('ui.auto.test_special')}</div>
        ...
        <div class="settings_section">
            ...
            <div class="ta_section_note">${t('ui.no_role_selected')}</div>
            <select id="ta_auto_character_select" class="text_pole" multiple></select>
        </div>
    ...
    async function initUI() {
        ...
        $('#ta_btn_preview').on('click', () => {
         ...
         $('#ta_btn_test_special').on('click', async () => {
            const ctx = SillyTavern.getContext();
            if (!ctx.getCurrentChatId()) {
            ...
            if (annivs.length > 0) {
                ...
                desc = t('toast.test_special_placeholder');
            }

            toastr.info(t('toast.generating_test_special'));
            const charName = ctx.name2 || '角色';
            const prompt = getAutoPromptByType('special', { charName, desc });
            const ok = await sendAsCharacter(prompt);
            if (ok) {
                if (lastAutoSaveFailed) {
                    ...
                    toastr.success(t('toast.test_special_sent'));
                }
            } else {
        ...
        $('#ta_btn_test_idle').on('click', async () => {
            ...
            toastr.info(t('toast.generating_test_idle'));
            const charName = ctx.name2 || '角色';

            const now = new Date();
            const gap = lastUserMessageTime ? calcGap(lastUserMessageTime, now.getTime()) : null;
            const gapStr = (gap && (gap.hours > 0 || gap.minutes > 0)) ? formatGapText(gap) : t('toast.test_idle_default_gap');
            const timeLine = t('prompt.current_time', { date: fmtDate(now), time: fmtTime(now) }) + `（${timePeriod(now.getHours(), now.getMinutes())}）`;
            const weatherLines = buildWeatherLines();
            const weatherText = weatherLines.length > 0 ? `\n${t('prompt.weather_info')}${weatherLines.join('；')}` : '';
            ...
            if (ok) {
                if (lastAutoSaveFailed) {
                    ...
                    toastr.success(t('toast.test_idle_sent'));
                }
            } else {
                toastr.error(t('toast.gen_failed', { error: lastAutoGenError || t('toast.api_check') }));
            }
        ...
        $('#ta_btn_diag_copy').on('click', async () => {
            try {
        ...
        $('#ta_btn_diag_download').on('click', () => {
            try {
                const diag = collectDiagnostics();
                const text = diagnosticsToText(diag);
                downloadDiagnostics(text);
                toastr.success('诊断文件已下载');
            } catch (e) {
                toastr.error(`下载失败：${String(e && e.message ? e.message : e || '')}`);
            }
        });
    ...
    function refreshCharacterList() {
        const settings = getSettings();
        const $sel = $('#ta_auto_character_select');
        const chars = SillyTavern.getContext().characters || [];

        $sel.empty();

        if (chars.length === 0) {
            $sel.append(`<option value="">${t('ui.no_roles')}</option>`);
            return;
        }
        ...
        chars.forEach((ch) => {
            ...
            if (!key) return;
            const selected = settings.autoMessageCharacters.includes(key) ? 'selected' : '';
            $sel.append(`<option value="${escHtml(key)}" ${selected}>${escHtml(ch.name || key)}</option>`);
        });

        $sel.off('change').on('change', function () {
            const vals = $(this).val();
            settings.autoMessageCharacters = Array.isArray(vals) ? vals : [];
            saveSettings();
        });
    }
    ...
    function refreshInjectCharacterList() {
        const settings = getSettings();
        const $sel = $('#ta_inject_character_select');
        const chars = SillyTavern.getContext().characters || [];
        ...
        chars.forEach((ch) => {
            const key = ch.avatar || ch.name;
            if (!key) return;
            const selected = settings.injectCharacters.includes(key) ? 'selected' : '';
            $sel.append(`<option value="${escHtml(key)}" ${selected}>${escHtml(ch.name || key)}</option>`);
        });

        $sel.off('change').on('change', function () {
            const vals = $(this).val();
            settings.injectCharacters = Array.isArray(vals) ? vals : [];
            saveSettings();
        });
    }
